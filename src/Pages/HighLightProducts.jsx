...
const HighLightProducts = () => {
  ...
  const renderProduct = (product, index) => {
    ...
    const {_id, title, image1, price } = product;

    return (
      <div
        className={`bg-white border border-solid relative group  border-gray-100 md:flex justify-center items-center shadow-md rounded-xl overflow-hidden p-6 ${
      ...
      >
        <div className="relative group md:w-96 w-auto p-4 border bg-white rounded-xl  overflow-hidden">
          ...
          {/* <div className="absolute inset-0 bg-black bg-opacity-40 flex items-center justify-center opacity-0 translate-y-5 group-hover:opacity-100 group-hover:translate-y-0 transition duration-300 ease-in-out">
            <Link  to={`/ProductDetails/${_id}`} className="bg-white text-black font-semibold py-2 px-4 rounded-full shadow-md hover:bg-gray-200 transition">
              View Details
            </Link>
          </div> */}
        </div>
